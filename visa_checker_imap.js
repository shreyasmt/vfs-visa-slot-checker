...
/**
 * VFS Global Visa Slot Checker - IMAP Version
 * Uses Gmail IMAP instead of OAuth - simpler setup with App Password
 */
...
class VisaSlotChecker {
  ...
  async getVerificationCode(maxAttempts = 10) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        const code = await this.checkEmail();
        if (code) {
  ...
  /**
   * Check email for verification code using IMAP
   */
  async checkEmail() {
    return new Promise((resolve, reject) => {
      ...
      imap.once('ready', () => {
        imap.openBox('INBOX', false, (err, box) => {
          ...
          // Search for unread emails from VFS Global in last 5 minutes
          const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
          
          imap.search([
            'UNSEEN',
            ['FROM', 'noreply@vfsglobal.com'],
            ...
            fetch.on('message', (msg) => {
              msg.on('body', (stream) => {
                simpleParser(stream, async (err, parsed) => {
                  ...
                  const codeMatch = text.match(/(?:code|OTP)[:\s]*([A-Z0-9]{6})/i);
  ...
  async run(email, password) {
    try {
      console.log('=== VFS Global Visa Slot Checker (IMAP Version) ===\n');
...
if (require.main === module) {
  ...
  if (!vfsEmail || !vfsPassword || !gmailUser || !gmailAppPassword) {
    console.error('Please set these environment variables:');
    console.error('  VFS_EMAIL - Your VFS Global email');
    console.error('  VFS_PASSWORD - Your VFS Global password');
    console.error('  GMAIL_USER - Your Gmail address');
    console.error('  GMAIL_APP_PASSWORD - Your Gmail app password');
    process.exit(1);
  }

  const checker = new VisaSlotChecker(gmailUser, gmailAppPassword);
  checker.run(vfsEmail, vfsPassword)
    .then(() => process.exit(0))
    .catch(error => {
